...
async function doDownload(btnSelector, extraEquipment) {
  ...
  let unitId = `GFL-${bitmaskId}`;
  if (unitId.length > 31) unitId = unitId.slice(0, 31);
...
function generateEntitiesXml(unitId, dolls, bitmaskId) {
  const entities = dolls
    .map((d) => {
      const e = d.entity;
      const o = dollOverrides[d.id] || {};
      let entityName = `GFL-${bitmaskId}-${e.name.replace("GIRL-", "")}`;
      if (entityName.length > 31) entityName = entityName.slice(0, 31);
